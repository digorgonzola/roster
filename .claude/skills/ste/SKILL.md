---
name: ste
description: Write prose in ASD-STE100 Simplified Technical English to cut AI slop. Use for docs, READMEs, PR text, release notes, error messages, and comments — not code, marketing, or essays.
---
...
Run the deterministic linter on any prose file to get a violations-per-100-words score:

```bash
python3 .claude/hooks/ste-lint.py path/to/file.md
```

Lint the comments inside a code file with `--comments`. The linter extracts only the comment prose (quote-aware, so a `#` or `//` inside a string or URL is not treated as a comment):

```bash
python3 .claude/hooks/ste-lint.py --comments path/to/workflow.yml
```

The linter also runs as a PostToolUse hook after every Write/Edit. Prose files (`.md`, `.mdx`, `.txt`) lint whole. Code files (`.yml`, `.ts`, `.sh`, Dockerfile, and more) lint their comments only. The score prints automatically. Lint before and after revision to measure improvement.
